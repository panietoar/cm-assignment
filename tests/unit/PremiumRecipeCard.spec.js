...
describe("PremiumRecipeCard.vue", () => {
  describe("given I pass the sample props", () => {
    ...
    test("it renders the recipe name", () => {
      expect(wrapper.find(".card-info__heading").text()).toBe(
        propsData.recipe.name
      );
    });

    test("it renders the total of ratings", () => {
      expect(wrapper.find(".ratings-total").text()).toBe(
        `${propsData.recipe.totalRatings} ratings`
      );
    });

    test("it renders the cooking time", () => {
      expect(wrapper.find(".details__cook-time").text()).toBe(
        `${propsData.recipe.cookingTime} min`
      );
    });

    test("it renders the calories", () => {
      expect(wrapper.find(".details__calories").text()).toBe(
        `${propsData.recipe.calories} Calories`
      );
    });
  ...
  describe("given I pass a truthy favorite prop", () => {
    ...
    test("it only renders the filled heart", () => {
      expect(
        wrapper.find(".card-image__favorite--filled").isVisible()
      ).toBeTruthy();
      expect(
        wrapper.find(".card-image__favorite--outline").exists()
      ).toBeFalsy();
    });
  });
  ...
  describe("given I pass a falsy favorite prop", () => {
    ...
    test("it only renders the outline heart", () => {
      expect(
        wrapper.find(".card-image__favorite--filled").exists()
      ).toBeFalsy();
      expect(
        wrapper.find(".card-image__favorite--outline").isVisible()
      ).toBeTruthy();
    });
  });
});
